...
const Demo106 = () => {
  ...
  const elem5 = <p><textarea id="note" rows="6" cols="100" value={noteContent}></textarea></p >;  //为textarea设置一个id
  //直接定义一个超链接，只有一行。
  //也可以将各个React元素变量组合起来定义为一个超链接
  const link = <a key="name" href=" " onClick={toggleNote}>{elem1}{elem3}{elem4}</a >;
  //定义一个按钮元素变量
  const btn = <button style={{ marginBottom: 10 }} onClick={toggleNote}>{showNote ? '点击隐藏简历' : '点击显示简历'}</button>;
  //输出各个元素变量，函数组件无需render
  return (
