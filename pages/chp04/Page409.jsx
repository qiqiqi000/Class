...
import { Panel, SideMenu, LinkButton, Dialog, ButtonGroup, Layout, LayoutPanel, Menu, MenuItem, MenuButton, SubMenu, MenuSep } from 'rc-easyui';
...
export default class Page407 extends React.Component {
      ...
      return (
        <Menu noline>
          <div style={{ padding: '10px' }}>
            <img src=" require('../../images/easyuilogo1.png" style={{ width: 150, height: 50 }} />
          </div>
        </Menu>
      )
    }
    ...
    handleMessage(type) {
      console.log(type);
      let icon='info;error;Alert;question;warning'
      this.messager.alert({
        title: "Warning",
        icon: "warning",
        ...
        <div>
          <Layout style={{ width: '100%', height: '100%', position:'absolute' }}>
            <LayoutPanel region="north" className="dialog-toolbar" style={{ height: 40 }}>
                <LinkButton onClick={this.handleClick.bind(this)} style={{ marginBottom: 3 }} plain>收缩菜单</LinkButton>
                <LinkButton onClick={this.handleMessage.bind(this,'info')} style={{ marginBottom: 3 }} plain>打开会话框</LinkButton>
                <LinkButton onClick={this.handleConfirm.bind(this)} style={{ marginBottom: 3 }} plain>打开确认框</LinkButton>
                <LinkButton onClick={()=>this.win1.open()} style={{ marginBottom: 3 }} plain>打开表单窗体</LinkButton>
                <LinkButton onClick={()=>this.win2.open()} style={{ marginBottom: 3 }} plain>打开表格窗体</LinkButton>
                <LinkButton onClick={this.confirm.bind(this)}>Confirm</LinkButton>
                <MenuButton text="帮助" plain iconCls="icon-help" menu={this.helpMenu}></MenuButton>
            </LayoutPanel>
