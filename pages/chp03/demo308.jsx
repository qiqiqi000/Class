...
import {MyCheckbox, MyCombobox, MyInput, MyRadiogroup, MyModal} from '../../api/common.js';
...
export default class Page308 extends Component {
  ...
  async componentDidMount(){ //初始状态时提取第1页数据
    ...
    //初始化时根据省份的初值生成城市的选项 
    // let initialValue = document.getElementById('regionid'); 
    // this.cityid.setState({sqlparams: {"parentnodeid": initialValue.value}, items: null});    
  }
    ...
    if (action === 'add') {
      for (let key in row) {
        if (this[key]){
          ...
          else if (this[key].state.classtype=='combobox' && this[key].state.items?.length>0) this[key].setState({value: this[key].state.items[0].value, readOnly: false});
      ...
      //省份变化后重新提取数据,下拉框第一个选项时(北京市)也应该触发加载选项的数据
      //console.log(777,row.regionid,this.regionid.state.value,this.regionid.state.items[0])
      //if (row.regionid !== this.regionid.state.value) {  //换行后this.regionid.state.value为初始值，MyCombobox中初值为空时设置了一个_vvv值
  ...
  handleDeleteClick = (rowindex) => {
    ...
    this.deletemodal.setState({open:true, message:'是否确定删除这个客户？【'+row.companyname+'】'});
  }
  ...
  setHeaders=()=>{  ///hhhhhhh设置表头
    ...
    for (let index = 0; index < fixedColumns.length; index ++){
      ...
      if (item.field === '_button' && rowSelectionType=='checkbox') headerCells.push(<th key={'th1_'+index} className='fixedHeaderStyle' style={{border: cellBorder, left:item.left, width: item.width, minWidth:item.width}}><input key={'button_'+index} type={rowSelectionType} /></th>)
      else if (item.field === '_action' && item.left !== undefined) headerCells.push(<th key={'th2_'+index} className='fixedHeaderStyle' style={{border: cellBorder, left:item.left, width: item.width, minWidth:item.width}}></th>)
      else if (item.field !=='_action') headerCells.push(<th key={'th3_' + index} className='fixedHeaderStyle' style={{border: cellBorder, left:item.left, width: item.width, minWidth:item.width}}>{item.title}</th>)
    }    
    //定义非固定列表头
    for (let index = 0; index < columns.length; index ++){
      let item = columns[index];
      headerCells.push(<th key={'th4_' + index} className='headerStyle' style={{border: cellBorder, width: item.width, minWidth:item.width}}>{item.title}</th>);
    }
    //定义右边固定列表头
    for (let index = 0; index < fixedColumns.length; index ++){
      let item = fixedColumns[index];
      if (item.field==='_action' && item.right !== undefined){
        headerCells.push(<th key={'th5_'+index} className='fixedHeaderStyle' style={{border: cellBorder, right:item.right, width: item.width, minWidth:item.width}}></th>)
      } 
    }
    return (<tr style={{cursor:'pointer', lineHeight: rowheight+'px', height: rowheight}}>{headerCells}</tr>);
  }
  ...
  setCellData= ()=>{ //生成表格数据
    ...
    let rows = [];  //行元素数组，所有的行都存储在这里
    for (let rowindex = 0; rowindex < data.length; rowindex ++){
      let row = data[rowindex];
      let cells = []; //一行中的各个单元格都存储在这里
      //定义左边固定列的表体内容
      ...
      rows.push(<tr key={"row_"+rowindex} className='table-link' style={{cursor:'pointer', height: rowheight, lineHeight: rowheight+'px'}}>{cells}</tr>);
    }
    return rows;
  }
  ...
  render() {
    ...
    return (
      <div id="mainpage" className="layout-body">
        ...
        {/*定义一个类似的窗体，用div*/}
        {/* <div className='myFormStyle' style={{position:'absolute', top:myForm1.position.y, left:myForm1.position.x, width:myForm1.isVisible? myForm1.width:1, height:myForm1.isVisible? myForm1.height:1}}> */}
        <div className='myFormStyle' style={{position:'absolute', top:myForm1.position.y, left:myForm1.position.x, width:myForm1.width, height:myForm1.height, zIndex:myForm1.isVisible? 1 : -1}}>
        <div className="layout-body">        
         ...
         <MyModal ref={ref=>this.deletemodal=ref} width='400' height='140' title='系统提示' message={"是否确认修改图书价格"} onConfirm={this.handleDeleteConfirm} />
      </div>
    );
  }
}
