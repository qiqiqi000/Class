...
const rowheight=42;
export default class Page508 extends React.Component {
  constructor(props) {
    super(props);
    this.state = {  //写属性
       ...
       setTimeout(()=>{
         if (rs.rows.length>0){
            for (var i=0; i<rs.rows.length; i++){
              if (rs.rows[i].isparentflag>0) this.myTree1.collapseNode(rs.rows[i]);
            } 
         } 
         this.myTree1.expandNode(rs.rows[0]);
      ...
      <div>
        <Layout style={{ width: '100%', height: '100%',position:'absolute' }}>          
          <LayoutPanel region="west" style={{ width:300 }} split>
             <Tree data={this.state.myTree1.data} checkbox={false} border={false} ref={node => this.myTree1 = node} style={{ overflow:'auto',width:'100%', height:'100%', position:'absolute' }}
          ...
          <LayoutPanel region="center" style={{ height:'100%' }}>
            <Layout>
              ...
              <div ref={ref => this.myGridPanel = ref} style={{ height: '100%', width: '100%', position: 'absolute', visibility: this.state.hidegridpanel? 'hidden':'visible'}}>
                <Panel ref={ref => this.GridPanel = ref} xclosed={this.state.hidegridpanel} border={false} style={{ height: '100%', width: '100%', position: 'absolute' }}>
                  <DataGrid ref={ref => this.myGrid1 = ref} columnResizing pagination CheckBox border={false} style={{ width: '100%', height: '100%', position: 'absolute' }}
                    ...
                    xonPageChange={({ pageNumber, pageSize }) => {console.log(101); const grid = this.state.myGrid1; this.setState({ myGrid1:{...grid, reloadflag:1} }); this.loadGridData(pageNumber, pageSize);}}
                ...
                <div ref={ref => this.myFormPanel = ref} style={{ height: '100%', width: '100%', position: 'absolute', visibility: !this.state.hidegridpanel? 'hidden':'visible'}}>
                  <Panel ref={ref => this.FormPanel = ref} closed={this.state.hideformpanel}  border={false}  style={{ height: '100%', width: '100%', position: 'absolute' }}>
                    <Tabs ref={ref => this.myTabs = ref}  border={false} style={{ height: '100%', width: '100%', position: 'absolute' }}  tabPosition='top'>
                      ...
                      <TabPanel ref={ref => this.myTab2 = ref} key="myTab2" title="图片上传" style={{ overflow: 'auto', height: '100%', width: '100%', position: 'absolute' }}>
                          <MyFileUpload ref={ref => this.photopath = ref} params='imagepath;选择文件;200;120' 
                           maxsize='100' layout='span' data={row.photopath} filetag={row.productid} filepath='\\mybase\\resources' />
                      </TabPanel>
                    </Tabs>
                  </Panel>
                </div>
