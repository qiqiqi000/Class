...
//函数实现非路由组件编程式跳转功能
const buttonStyle={
  marginTop:10, marginLeft:8, width:182, height:30 
}
function anonyCom(A) {
  return (props) => {
    ...
    return (
      <Layout>
         <Sider theme='light' width={200} style={{margin:0, padding:0, height:'100%', position:'relative', marginLeft:0, padding:0, borderRight:'1px solid #95B8E7'}}>
            <div><Button type="primary" style={buttonStyle} onClick={() => this.handleClick("Page701")}>实例701</Button></div>
            <div><Button type="primary" style={buttonStyle} onClick={() => this.handleClick("Page702")}>实例702</Button></div>
            <div><Button type="primary" style={buttonStyle} onClick={() => this.handleClick("Page703")}>实例703</Button></div>
         </Sider>
...
export default anonyCom(App); //将组件包装成可编程式跳转的路由组件